...
async function func(): Promise<void> {
	...
	const yakuza_contributors_str = await deployments.readDotFile(
		'.yakuza_contributors.json',
	);
	...
	const totalContributions = yakuza_contributors.reduce(
		(sum, contributor) =>
			sum.add(BigNumber.from(contributor.yakuzaSubscription.totalContribution)),
		BigNumber.from(0),
	);

	// Calculate and assign rewards proportionally based on contribution
	for (const contributor of yakuza_contributors) {
		const contributionAmount = BigNumber.from(
			contributor.yakuzaSubscription.totalContribution,
		);
		...
		const reward = contributionAmount
			.mul(REWARDS_TO_DISTRIBUTE)
