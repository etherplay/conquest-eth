...
async function main() {
	...
	const transactionsFromFile: TransactionJSONObject[] = JSON.parse(
	...
	const endTime =
		transactionsFromFile[transactionsFromFile.length - 1].timestamp;
	const duration = endTime - startTime;
	console.log(
		`processing ${transactionsFromFile.length} transaction for across ${duration / 3600} hours`,
	);
			...
			if (lastBlockTime > 0) {
				...
				try {
					await provider.send(`evm_setNextBlockTimestamp`, [
						currentStartTime + delayBetweenBlock,
					]);
				} catch (e) {
					console.error(e);
				}
