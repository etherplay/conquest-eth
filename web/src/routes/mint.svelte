<script lang="ts">
  ...
  async function mint(amount: number | undefined) {
    try {
      if (amount === 0) {
        throw new Error('zero amount');
      }
      if (wallet.address && wallet.contracts) {
        const numTokens = BigNumber.from(amount).mul('1000000000000000000');

        const nativeTokenAmount = numTokens
          .mul('1000000000000000000')
          .div(initialContractsInfos.contracts.PlayToken.linkedData.numTokensPerNativeTokenAt18Decimals);

        const balanceBefore = $myTokens.playTokenBalance;
        const tx = await wallet.contracts.PlayToken.mint(wallet.address, numTokens, {value: nativeTokenAmount});
        waiting_tx = {balanceBefore, txMinted: false};
        await tx.wait();
        waiting_tx = {balanceBefore, txMinted: true};
        while (waiting_tx) {
          if ($myTokens.playTokenBalance.gt(waiting_tx.balanceBefore)) {
      ...
      {:else}
        ...
        {:else}
          <p class="m-2 text-center">
            Current Balance: {$myTokens.playTokenBalance.div('10000000000000000').toNumber() / 100}
          </p>
        {/if}
        ...
        <input
          ...
          placeholder="all"
        />
        <Button class="block w-max-content m-4" label="claim" disabled={waiting_tx} on:click={() => mint(amount)}>
          mint
        </Button>
      {/if}
    </div>
