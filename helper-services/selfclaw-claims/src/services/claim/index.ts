...
import type {ClaimRequest, ClaimResponse, ClaimSuccessResponse, ClaimErrorResponse, StatusResponse} from './types.js';
import {buildClaimMessage, verifySignature, validateTimestamp, validateWalletAddress} from './verification.js';
...
export async function processClaim(
  ...
  const signatureValid = await verifySignature(request.publicKey, request.signature, message);
  ...
  const verification = await checkVerification(request.publicKey, config.selfClawApiUrl);
  ...
  try {
    txHash = await transferTokens(
      ...
      config.rpcUrl,
      config.chainId,
      config.tokenDistributorAddress && config.nativeTokenAmount
        ? {
            address: config.tokenDistributorAddress,
            nativeTokenAmount: config.nativeTokenAmount,
          }
        : undefined,
    );
  } catch (error) {
 ...
 */
function createError(error: ClaimErrorResponse['error'], message: string): ClaimErrorResponse {
  return {
    success: false,
    error,
    message,
