...
`;
	...
	const planetsPerSponsors: {
		[sponsor: string]: {x: number; y: number; location: string}[];
	} = {};
	try {
		...
		for (const planet of rewardsPlanets) {
			planetsPerSponsors[planet.sponsor] =
				planetsPerSponsors[planet.sponsor] || [];
	...
	let rewardsAlreadyGiven: {
		[sponsor: string]: {
			[address: string]: {
				given: {tx: string; planets: string[]; units: number}[];
			};
		};
	} = {};

	try {
		rewardsAlreadyGiven = JSON.parse(
			await deployments.readDotFile('.planets_rewards_given.json'),
		);
	} catch {}
	...
	const exitingDone = data.planetExitEvents.filter(
		(v) =>
			!v.interupted &&
			!v.complete &&
			now - parseInt(v.exitTime) > 3 * 24 * 3600,
	);
	...
	const winners: {
		[id: string]: {planets: string[]} & {[token: string]: number};
	} = {};
	...
	for (const planetExited of exited) {
		if (!planetsCounted[planetExited.planet.id]) {
			...
			if (
				rewardsAlreadyGiven[sponsor] &&
				rewardsAlreadyGiven[sponsor][planetExited.owner.id]
			) {
				for (const givenElem of rewardsAlreadyGiven[sponsor][
					planetExited.owner.id
				].given) {
			...
			if (!alreadyCounted) {
				winners[planetExited.owner.id][sponsor] =
					winners[planetExited.owner.id][sponsor] || 0;
	...
	for (const planetHeld of held) {
		if (!planetsCounted[planetHeld.id] && planetHeld.owner) {
			...
			if (
				rewardsAlreadyGiven[sponsor] &&
				rewardsAlreadyGiven[sponsor][planetHeld.owner.id]
			) {
				for (const givenElem of rewardsAlreadyGiven[sponsor][
					planetHeld.owner.id
				].given) {
			...
			if (!alreadyCounted) {
				winners[planetHeld.owner.id][sponsor] =
					winners[planetHeld.owner.id][sponsor] || 0;
	...
	for (const loc of planets) {
		if (!planetsCounted[loc]) {
			console.log(
				`not counted: ${locationToXY(loc).x}, ${locationToXY(loc).y}`,
			);
		}
	}

	await deployments.saveDotFile(
		`.planets_rewards_${sponsor}.json`,
