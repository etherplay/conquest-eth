...
`;
	...
	const REWARD_RATE_millionth = BigNumber.from(
		RewardsGenerator.linkedData.rewardRateMillionth,
	);
	...
	function _updateGlobal(g: GlobalPoints, time: BigNumber): GlobalPointsBN {
		...
		const totalRewardPerPointAllocatedSoFar =
			_global.totalRewardPerPointAtLastUpdate.add(extraTotalRewardPerPoint);
	...
	const highscoreQueryResult: QueryResult | undefined =
		await theGraph.complexQuery<QueryResult>(queryString, {
			variables: {},
			list: {
				path: 'owners',
			},
	...
	if (data.data) {
		...
		highscores = data.data.owners
			...
			.map((p) => {
				...
				const points_fixed_toWithdraw = BigNumber.from(
					p.points_fixed_toWithdraw,
				);
				const points_shared_rewardsToWithdraw = BigNumber.from(
					p.points_shared_rewardsToWithdraw,
				);
				...
				const {totalRewardPerPointAtLastUpdate} = _updateGlobal(
					globalData,
					BigNumber.from(time),
				);
				...
				return {
					...
					fixed_score:
						fixed_amount.mul(1000000).div(DECIMALS_18).toNumber() / 1000000,
					pool_score:
						pool_amount.mul(1000000).div(DECIMALS_18).toNumber() / 1000000,
					account,
					globalData,
				};
			})
		...
		const rewards = scores.map((player) => {
			const rewardAmount = TOTAL_REWARD_USD.mul(
				BigNumber.from(player.score).mul(1000000),
			).div(BigNumber.from(totalScore).mul(1000000));
			return {
