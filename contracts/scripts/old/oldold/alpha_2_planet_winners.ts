...
if (discord) {
	discordMembers = JSON.parse(
		fs.readFileSync('discord_members.json').toString(),
	);
...
const rewards = [
	...
	{
		sponsor: 'xaya',
		planets: ['-80,-105', '50,39', '85,24', '138,28', '45,41', '-110,105'],
	},
	{
		sponsor: 'da',
		planets: ['-98,-122', '-114,-11', '73,13', '-28,156', '-78,-99', '112,-76'],
	},
];
...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	const playerArray: Player[] = JSON.parse(
		await deployments.readDotFile('.players.json'),
	);
	...
	try {
		winnersArray = JSON.parse(
			await deployments.readDotFile('.alpha_2_winners.json'),
		);
	} catch (e) {}
	for (const winner of Object.keys(winners)) {
		...
		let discordName = '';
		if (discord) {
			const claimFound = claims.find(
				(v) =>
					v.address.toLowerCase() === winners[winner].introducer.toLowerCase(),
			);
			if (claimFound) {
