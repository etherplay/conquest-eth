...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	}[] = JSON.parse(await deployments.readDotFile('.players_captures.json'));

	const players_stakes: {
		playerAddress: string;
		amountStaked: string;
		numPlanetsStaked: number;
	}[] = JSON.parse(await deployments.readDotFile('.players_stakes.json'));
	...
	const totalWeightedPoints =
		totalAmountCaptured * BigInt(captureWeight) +
		totalAmountStaked * BigInt(stakeWeight);

	for (const capture of players_captures) {
		const amountCaptured = BigInt(capture.amountCaptured);
		const capturePoints = Number(
			(amountCaptured *
				BigInt(captureWeight) *
				BigInt(totalMeritsPointPerWeek)) /
				totalWeightedPoints,
		);
		playerMerits[capture.playerAddress] =
			(playerMerits[capture.playerAddress] || 0) + capturePoints;
	}

	for (const stake of players_stakes) {
		const amountStaked = BigInt(stake.amountStaked);
		const stakePoints = Number(
			(amountStaked * BigInt(stakeWeight) * BigInt(totalMeritsPointPerWeek)) /
				totalWeightedPoints,
		);
		playerMerits[stake.playerAddress] =
			(playerMerits[stake.playerAddress] || 0) + stakePoints;
	}
	...
	do {
		...
		if (excessPoints > 0 && uncappedPlayers.length > 0) {
			for (const address of uncappedPlayers) {
				...
				const pointsToAdd = Math.min(
					maxPointsPerPlayer - currentPoints,
					Math.floor(excessPoints * proportion),
				);
		...
		excessPoints > 0 &&
		totalDistributedPoints < totalMeritsPointPerWeek
	);
