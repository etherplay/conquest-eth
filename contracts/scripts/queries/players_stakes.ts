...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	const toBlock =
		args[1] === 'latest' ? latestBlockNnmber - 12 : parseInt(args[1]);
...
`;
	...
	for (const event of planetStakeEvents) {
		...
		const playerCapture = players_stakes.find(
			(player) => player.playerAddress === playerAddress,
		);
		if (playerCapture) {
			playerCapture.amountStaked = (
				BigInt(playerCapture.amountStaked) + BigInt(amountStaked)
			).toString();
	...
	await deployments.saveDotFile(
		'.players_stakes.json',
		JSON.stringify(players_stakes, null, 2),
	);
	//   await deployments.saveDotFile('.fleetArrivedEvents.json', JSON.stringify(fleetArrivedEvents, null, 2));
	console.log({
		numPlanetStakeEvents: planetStakeEvents.length,
		numPlayers: players_stakes.length,
	});
}
