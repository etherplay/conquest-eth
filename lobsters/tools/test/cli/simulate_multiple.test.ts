...
describe('CLI - Simulate Multiple Fleets', () => {
	...
	describe('simulate_multiple', () => {
		it(
			'should simulate multiple fleets attacking the same target',
			async () => {
		...
		it(
			'should return individual fleet outcomes',
			async () => {
				expect(testPlanets.length).toBeGreaterThanOrEqual(3);
		...
		it(
			'should return final combined outcome',
			async () => {
				expect(testPlanets.length).toBeGreaterThanOrEqual(3);
		...
		it(
			'should accept custom arrival time',
			async () => {
				expect(testPlanets.length).toBeGreaterThanOrEqual(3);
		...
		it(
			'should work with a single fleet (edge case)',
			async () => {
		...
		it(
			...
			async () => {
				...
				const {stdout, stderr, exitCode} = await invokeCliCommand([
					...
					'--fleets',
					JSON.stringify([
						{from: {x: 999999, y: 999999}, quantity: 1000},
					]),
					'--to',
					`${target.location.x},${target.location.y}`,
				]);
		...
		it(
			'should require at least one fleet',
			async () => {
		...
		it(
			'should require positive quantity for each fleet',
			async () => {
				expect(testPlanets.length).toBeGreaterThanOrEqual(2);
				...
				const {exitCode} = await invokeCliCommand([
					...
					'--fleets',
					JSON.stringify([
						{from: {x: fromPlanet.location.x, y: fromPlanet.location.y}, quantity: 0},
					]),
					'--to',
					`${target.location.x},${target.location.y}`,
				]);
