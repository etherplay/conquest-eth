...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	for (const recipient of recipients) {
		const reward = BigNumber.from(recipient.numTokens).mul(
			'1000000000000000000',
		);
	...
	const etherAmount = etherAmounts.reduce(
	...
	console.log({
		balancesBefore: balancesBefore.map((v) =>
			v.div('1000000000000000000').toString(),
		),
		balancesAfter: balancesAfter.map((v) =>
			v.div('1000000000000000000').toString(),
		),
	});
}
