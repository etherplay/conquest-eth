...
async function func(file: string): Promise<void> {
	...
	const numTokensPerNativeTokenAt18Decimals = BigNumber.from(
		PlayToken.linkedData.numTokensPerNativeTokenAt18Decimals,
	);
	...
	for (const address of Object.keys(tokendue)) {
		...
		totalValue = totalValue.add(
			amount
				...
				.div(numTokensPerNativeTokenAt18Decimals),
		);
	}
