...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	const recipients: {address: string; tokenUnitGivenSoFar: number}[] =
		JSON.parse(await hre.deployments.readDotFile('.airdrop.json'));
	...
	for (const recipient of recipients) {
		const tokenGiven = BigNumber.from(recipient.tokenUnitGivenSoFar).mul(
			'1000000000000000000',
		);
