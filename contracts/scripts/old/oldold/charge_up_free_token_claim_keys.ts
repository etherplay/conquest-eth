...
async function func(hre: HardhatRuntimeEnvironment): Promise<void> {
	...
	const lists: {list: string[]; used: boolean}[] = JSON.parse(
		await deployments.readDotFile('.free_token_list.json'),
	);
	...
	for (const claimLink of list) {
		const privateKey = claimLink.slice(
			'https://defcon.conquest.etherplay.io/#tokenClaim='.length,
		);
