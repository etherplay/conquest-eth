...
const setup = deployments.createFixture(async () => {
	...
	const contracts = {
		...
		PaymentWithdrawalGateway: <PaymentWithdrawalGateway>(
			await ethers.getContract('PaymentWithdrawalGateway')
		),
	};
...
describe('PaymentWithdrawalGateway', function () {
	it('player can withdraw ETH via message and emit the corresponding event', async function () {
		...
		const signature = await agentServiceWallet.signer.signMessage(
			ethers.utils.arrayify(dataHash),
		);

		await expect(
			players[0].PaymentWithdrawalGateway.withdraw(
				players[0].address,
				maxAmount,
				timestamp,
				signature,
	...
	it('player cannot withdraw ETH via invalid message', async function () {
		...
		const dataHash = ethers.utils.keccak256(data);
		const signature = await players[1].signer.signMessage(
			ethers.utils.arrayify(dataHash),
		);
	...
	it('player cannot withdraw ETH via same message', async function () {
		...
		await players[0].PaymentWithdrawalGateway.withdraw(
			players[0].address,
			maxAmount,
			timestamp,
			signature,
	...
	it('player can withdraw ETH twice past the interval', async function () {
		...
		const signature2 = await agentServiceWallet.signer.signMessage(
			ethers.utils.arrayify(dataHash2),
		);
	...
	it('gatewayOwner can change ownership of PaymentWithdrawalGateway', async function () {
		...
		await waitFor(
			gatewayOwner.PaymentWithdrawalGateway.transferOwnership(
				players[0].address,
			),
		);
	...
	it('gatewayOwner can change ownership of PaymentGateway', async function () {
		...
		await waitFor(
			gatewayOwner.PaymentWithdrawalGateway.transferPaymentGatewayOwnership(
				players[0].address,
			),
		);
	...
	it('random account cannot change ownership', async function () {
		...
		await expect(
			players[1].PaymentWithdrawalGateway.transferOwnership(players[0].address),
		).to.be.reverted;
	});
});
